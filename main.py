...
class LRY:
    ...
    def login(self,isUpdate):
        ...
        if moodlesession == '' or isUpdate:
            ...
            if res['msgcode']==-1:
                print('账号密码错误！！')
                return
            self.config.set('Cookie', 'name', res['name'])
            self.config.set('Cookie', 'dept', res['dept'])
            self.config.set('Cookie', 'account', res['account'])
            self.config.set('Cookie', 'phone', res['phone'])
            # 生成MoodleSession
        ...
        print('姓名:{}'.format(self.config.get('Cookie', 'name')))
        print('学号:{}'.format(self.config.get('Cookie', 'account')))
        print('学院:{}'.format(self.config.get('Cookie', 'dept')))
        print('\n')
    ...
    def study(self,sesskey,video_item):
        '''
        获取视频在线资源地址（用于获取视频真实时长，！！！以第一次上传进度为准，视频总时长不会被后面修改！！！）
    ...
    def auto_answer(self,sesskey,cmid):
        ...
        while True:
            ...
            for item in question_list:
                ...
                if title == None:
                    title = item.xpath('normalize-space(.//div[@class="qtext"]//p[@dir="ltr"][1])').replace('\xa0', ' ')
                for q in questions:
                    if id == q['id']:
                        end=True
                if not end:
        ...
        for i in questions:
            ...
            print(i)
            print("答案:[{}]".format(i['answer'][ans_index]))
    ...
    def getAnswer(self,question):
        ans_index = -1
        for item in self.题库:
            if item.__contains__(question['question']):
                index = self.题库.index(item)+1
                for ans in question['answer']:
                    if self.题库[index].__contains__(ans):
                        ans_index = question['answer'].index(ans)

                # 题目下一个没找到答案返回题目标题附近寻找 （题库有点乱，不整理了就这样了）
                if ans_index==-1:
                    index = index-1
                    for ans in question['answer']:
                        for add in range(4):
                            self.题库[index+add].__contains__(ans)
                            ans_index = question['answer'].index(ans)
        return ans_index
...
if __name__ == '__main__':
    ...
    if lry.isLogin:
        ...
        # index = eval(input("请输入所需学习课程序号以Enter确认:\n"))-1
        while True:
            print('--------------------------------------')
        ...
        if type == 1:
            ...
            for question in studyCourse['question_list']:
                pass
            for idx in range(len(studyCourse['question_list'])):
                print('--------------------------------------')
                print(studyCourse['question_list'][idx])
                isG = input("回车开始测验 [输入q跳过该章节]:")
                if isG != 'q':
